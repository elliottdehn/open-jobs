...
Chat produces a complete, aspirational structured draft from the first meaningful work direction. It follows a real posting’s structure: role facts, company overview, operating rhythm, opportunity, responsibilities, required skills and optional extras. Sensible optimistic defaults fill gaps; the conversation calls out important assumptions and asks focused refinement questions. Explicit preferences and manual edits take precedence. No placeholders or interview questions belong inside the JD. Nothing is embedded until the person clicks the approval button. Manual editing works if chat is unavailable. Chat has bounded context and output, a timeout, same-origin protection, and 30 requests per IP per 10 minutes. It uses Responses with `store:false`; no transcript is written to application storage or application logs on the server. The UI accurately discloses the AI transfer. Never send résumés, ranking labels, or notes to chat.
...
Search preparation transfers at most 48 postings or approximately 512 KiB per batch (oversized individual postings are isolated). Python receives only parsing text and relevant location lookups, and returns only facet metadata; full descriptions and vectors stay in JavaScript. Its worker is terminated after preparation, including errors, to release WebAssembly memory before the search UI opens. `test-preparation.mjs` covers bounded transfers, complete output, and failure cleanup.

Startup opens the nearest group first, then automatically extends the neighborhood in the background. Preparation reports its stage and completed posting count; individual worker requests time out after 45 seconds. The pinned Python runtime is copied from the npm dependency into versioned same-origin assets by `build:web`, so startup does not rely on a third-party CDN.

...
Writes <EXPORT_DIR>/web/seniority-model.json {recipe, classes, W, b, n, holdout} — the client applies
softmax(W·v + b) to titles that don't say, shows the top class as an estimate when it clears the threshold,
and falls back to "mid" (never stated in titles) otherwise.
Run: EXPORT_DIR=export/latest uv run scripts/train-seniority.py   (SAMPLE env: rows to scan, default 250000)"""

...
#        [--from STAGE]  resume from a stage (ingest pull ledger parquet diff tree estimators finalize history retention)
set -euo pipefail
cd "$(dirname "$0")/.."
...
STAGES=(ingest pull ledger parquet diff tree estimators finalize history retention)
...
for st in "${STAGES[@]}"; do
  ...
  case "$st" in
    ...
    finalize|history) has --skip-upload && { echo "--- $i/$N $st skipped (--skip-upload)"; continue; };;
  esac
